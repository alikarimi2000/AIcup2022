...
def shouldUpgradeDefence(view: GameState, activationThreshold: float) -> bool:
    pos=find_closest_type(view.location, MapType. AGENT)
    ...
    if percent(view.rounds, view.current_round) < activationThreshold \
        ...
        dist = abs(view.location[0] - pos[0]) + abs(view.location[1] - pos[1])
        if dist<view.ranged_attack_radius+2 and view.wallet//2 >= view.def_upgrade_cost:
            return True
       
   
    if percent(view.rounds, view.current_round) < 50 \
            and view.wallet >= view.def_upgrade_cost:    
        dist = abs(view.location[0] - pos[0]) + abs(view.location[1] - pos[1])
        if dist<view.ranged_attack_radius+2 and view.wallet//4 >= view.def_upgrade_cost:
            return True
